    ...
    try {
      if (tab === 'binance') {
        ...
        const formatted = await Promise.all(symbolsToFetch.map(async sym => {
          const res = await fetch(`https://finnhub.io/api/v1/quote?symbol=${sym}&token=d9s35a1r01qoo7o6j450d9s35a1r01qoo7o6j45g`);
          const data = await res.json();
          if (data.error) {
        ...
        const formatted = await Promise.all(symbolsToFetch.map(async s => {
          const res = await fetch(`https://finnhub.io/api/v1/quote?symbol=${s.sym}&token=d9s35a1r01qoo7o6j450d9s35a1r01qoo7o6j45g`);
          const data = await res.json();
  ...
  const fetchLivePrice = async (ticker, type) => {
    try {
      if (type === 'Crypto') {
        ...
        const res = await fetch(`https://finnhub.io/api/v1/quote?symbol=${ticker.toUpperCase()}&token=d9s35a1r01qoo7o6j450d9s35a1r01qoo7o6j45g`);
